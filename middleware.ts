...
async function getRouteTogglesFromSanity(): Promise<RouteToggle[]> {
  ...
  const query = '*[_type == "siteFeatureFlags"][0]{routes[]{routePath,enabled}}'
...
export default auth(async (req) => {
  ...
  if (isRouteDisabled(path, routeToggles)) {
...
export const config = {
  matcher: ["/leaders/:path*", "/((?!api|_next/static|_next/image|favicon.ico|robots.txt|sitemap.xml|opengraph-image).*)"],
}
