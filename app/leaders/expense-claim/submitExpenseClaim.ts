...
const MAX_COMBINED_ATTACHMENT_SIZE_BYTES = 4 * 1024 * 1024 // 4 MB combined
const MAX_TOTAL_ATTACHMENTS = 20

interface ExpenseClaimSubmitter {
  email?: string | null
  name?: string | null
  leaderName?: string | null
  isAuthorizedLeader?: boolean | null
}
...
export async function processExpenseClaimSubmission(
  formData: FormData,
  user: ExpenseClaimSubmitter | null | undefined
): Promise<SubmitExpenseClaimResult> {
  if (!user?.isAuthorizedLeader) {
    return { success: false, error: "Unauthorised" }
  }

  const applicantEmail = sanitiseEmail(user.email ?? "")
  const applicantName = sanitiseString(user.leaderName ?? user.name ?? "", 200)
  ...
  for (let i = 0; i < itemCount; i++) {
    ...
    if (rawReceipt instanceof File && rawReceipt.size > 0) {
      ...
      if (rawReceipt.size > MAX_FILE_SIZE_BYTES) {
        return {
          success: false,
          error: `Item ${i + 1}: receipt file exceeds the 4 MB limit.`,
        }
      }
      ...
      if (combinedAttachmentBytes > MAX_COMBINED_ATTACHMENT_SIZE_BYTES) {
        return {
          success: false,
          error: "Combined receipt attachments exceed the 4 MB limit. Please remove one or more files.",
        }
      }
      ...
      const isValidFile =
        ...
        (rawReceipt.type.startsWith("image/jpeg") && buffer[0] === 0xff && buffer[1] === 0xd8)
